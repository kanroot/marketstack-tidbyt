...
### URL TO GET AN API https://marketstack.com/signup/free
...
def get_percentage_with_two_decimals(last_price, previus_last_price):
    ...
    v = int(math.round(difference_percentage * 100))
    v = str(v)
    v = v[0:-2] + "." + v[-2:]
    v = str(float(v))
    return v
...
def get_data_select_period(request, colors, select_period, company_name):
    ...
    previus_last_price = list_data[1]
    ...
    company_name_clean = re.findall(str(r"^([^.])+"), company_name)
    return render.Root(
        render.Column(
            children=[
                render.Row(
                    ...
                    children=[
                        render.Column(
                            ...
                            children=[
                                render.Text(
                                    font="CG-pixel-3x5-mono", content=company_name_clean[0]
                                ),
                                render.Text(
